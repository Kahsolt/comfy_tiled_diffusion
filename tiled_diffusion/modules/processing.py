...
from .devices import *
from .extra_networks import *
from .lowvram import *
from .masking import *
from .script_callbacks import *
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    # if checkpoint_alisases.get(p.override_settings.get('sd_model_checkpoint')) is None:
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            #     try:
            #         step_multiplier = 2 if all_samplers_map.get(p.sampler_name).aliases[0] in ['k_dpmpp_2s_a', 'k_dpmpp_2s_a_ka', 'k_dpmpp_sde', 'k_dpmpp_sde_ka', 'k_dpm_2', 'k_dpm_2_a', 'k_heun'] else 1
            #     except:
            #         pass
            uc = get_conds_with_caching(get_learned_conditioning, negative_prompts, p.steps * step_multiplier, cached_uc)
            c = get_conds_with_caching(get_multicond_learned_conditioning, prompts, p.steps * step_multiplier, cached_c)
    ...
    if not p.disable_extra_networks and extra_network_data:
        deactivate(p, extra_network_data)

    # devices.torch_gc()

    res = Processed(p, output_images, p.all_seeds[0], '', comments="".join(["\n\n" + x for x in comments]), subseed=p.all_subseeds[0], index_of_first_image=index_of_first_image, infotexts=infotexts)
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        if self.enable_hr:
            if self.applied_old_hires_behavior_to != (self.width, self.height):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        self.sampler = create_sampler(self.sampler_name, self.sd_model)

        # latent_scale_mode = shared.latent_upscale_modes.get(self.hr_upscaler, None) if self.hr_upscaler is not None else shared.latent_upscale_modes.get(shared.latent_upscale_default_mode, "nearest")
        latent_scale_mode = None
        ...
        def save_intermediate(image, index):
            ...
            if not isinstance(image, Image.Image):
                image = sample_to_image(image, index, approximation=0)
        ...
        self.sampler = create_sampler(img2img_sampler_name, self.sd_model)
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        ...
        add_color_corrections = self.color_corrections is None
