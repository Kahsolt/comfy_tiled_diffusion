...
class Condition:
    ...
    def get_custom_cond(prompts:List[str], prompt, steps:int, styles=None) -> Tuple[Cond, ExtraNetworkData]:
        ...
        _, extra_network_data = parse_prompts([prompt])
    ...
    #     prompts, _ = parse_prompts(prompts)
    ...
    def reconstruct_cond(cond:Cond, step:int) -> Tensor:
        list_of_what, tensor = reconstruct_multicond_batch(cond, step)
        return tensor

    def reconstruct_uncond(uncond:Uncond, step:int):
        tensor = reconstruct_cond_batch(uncond, step)
        return tensor
...
def feather_mask(w:int, h:int, ratio:float) -> Tensor:
    ...
    return torch.from_numpy(mask).to(device, dtype=torch.float32)
